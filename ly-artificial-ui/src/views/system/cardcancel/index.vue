  ...
  <div class="app-container">
    ...
    <el-row :gutter="20">
      <el-col :span="20" :xs="24">
        ...
        <el-table v-loading="loading" :data="cardcancelList" @selection-change="handleSelectionChange">
          <el-table-column type="selection" width="50" align="center"/>

          <el-table-column label="解绑记录表ID" align="center" prop="cancelCardId"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        cancelCardId: [
          { required: true, message: '解绑记录表ID不能为空', trigger: 'blur' }
        ],
        annalCardnum: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listCardcancel(this.queryParams).then(response => {
          this.cardcancelList = response.data.records
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.cancelCardId)
    ...
    handleUpdate(row) {
      this.reset()
      const updateId = row.cancelCardId || this.ids
      getCardcancel(updateId.toString()).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.cancelCardId !== undefined) {
            updateCardcancel(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addCardcancel(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const deletedId = row.cancelCardId || this.ids
      this.$confirm('是否确认删除编号为"' + deletedId + '"的数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return deleteCardcancel(deletedId.toString())
      }).then(() => {
        this.getList()
