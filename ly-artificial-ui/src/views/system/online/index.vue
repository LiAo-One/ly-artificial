  ...
  <div class="app-container">
    ...
    <el-row :gutter="20">
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
          <el-form-item label="记录id" prop="onlineId">
            <el-input
              v-model="queryParams.onlineId"
              placeholder="请输入记录id"
              clearable
              size="small"
              style="width: 240px"
          ...
          <el-form-item label="通知名称" prop="deviceName">
            <el-input
        ...
        <el-table v-loading="loading" :data="onlineList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="记录id" align="center" prop="onlineId"
                           :show-overflow-tooltip="true"/>

          <el-table-column label="通知名称" align="center" prop="deviceName"
                           :show-overflow-tooltip="true"/>

          <el-table-column label="设备SN" align="center" prop="deviceSn"
                           :show-overflow-tooltip="true"/>

          <el-table-column label="设备状态" align="center" prop="student"
                           :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-tag v-if="scope.row.student === 0" type="success">上线</el-tag>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-col :span="12">
          <el-form-item label="通知名称" prop="deviceName">
            <el-input v-model="form.deviceName" placeholder="请输入通知名称"/>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="设备SN" prop="deviceSn">
        ...
        <el-col :span="12">
          <el-form-item label="设备状态" prop="student">
            <!--<el-input v-model="form.student" placeholder="请输入设备状态"/>-->
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.onlineId)
    ...
    handleUpdate(row) {
      this.reset()
      const updateId = row.onlineId || this.ids
      getOnline(updateId.toString()).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.onlineId !== undefined) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const deletedId = row.onlineId || this.ids
      this.$confirm('是否确认删除编号为"' + deletedId + '"的数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return deleteOnline(deletedId.toString())
      }).then(() => {
        this.getList()
