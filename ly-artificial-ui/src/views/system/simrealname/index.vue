  ...
  <div class="app-container">
    ...
    <el-row :gutter="20">
      <el-col :span="20" :xs="24">
        ...
        <el-table v-loading="loading" :data="simrealnameList" @selection-change="handleSelectionChange">
          <el-table-column type="selection" width="50" align="center"/>


          <el-table-column label="实名认证id" align="center" prop="simrealnameId"
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-col :span="12">
          <el-form-item label="状态" prop="state">
            <!--<el-input v-model="form.state" placeholder="请输入状态（0: 已实名 1: 未实名）"/>-->
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        state: [
          { required: true, message: '状态（0: 已实名 1: 未实名）不能为空', trigger: 'blur' }
        ],
        version: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listSimrealname(this.queryParams).then(response => {
          this.simrealnameList = response.data.records
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.simrealnameId)
    ...
    handleUpdate(row) {
      this.reset()
      const updateId = row.simrealnameId || this.ids
      getSimrealname(updateId.toString()).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.simrealnameId !== undefined) {
            updateSimrealname(this.form).then(response => {
              this.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addSimrealname(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const deletedId = row.simrealnameId || this.ids
      this.$confirm('是否确认删除编号为"' + deletedId + '"的数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return deleteSimrealname(deletedId.toString())
      }).then(() => {
        this.getList()
